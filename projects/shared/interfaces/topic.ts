...
export interface BpmnEligibility {
  score: number;
  hasParticipants: boolean;   // actors, roles, systems
  hasActivities: boolean;     // distinct tasks or actions
  hasDecisions: boolean;      // conditional steps like if/else
  hasSequence: boolean;       // Sequence/temporal order
  hasArtifacts: boolean;      // documents, data, APIs, resources
  recommendations: string[];  // Provide recommendations if something is missing or unclear
}
